local inp = require("util.input")
local key_time = require('keys.time')
local key_pulse = key_time.pulse
local key_mail = require('keys.mail')
local key_sc = require('keys.shortcut')
...
return {
  ...
  },
  {
    ...
    init = function()
      ...
      local cmdr = require("util.autocmd").autocmdr("Himalaya", true)
      cmdr({ "FileType" }, {
  ...
  },
  {
    ...
    opts = function(_, opts)
      opts.username = "ursa-major" -- needed to fetch profile data
      opts.base_url = "https://codestats.net" -- codestats.net base url
      opts.api_key = os.getenv("CODESTATS_API_KEY")
      opts.send_on_exit = true -- send xp on nvim exit
      opts.send_on_timer = true -- send xp on timer
      opts.timer_interval = 60000 -- timer interval in milliseconds (minimum 1000ms to prevent DDoSing codestat.net servers)
      opts.curl_timeout = 5 -- curl request timeout in seconds
    end
  },
}
