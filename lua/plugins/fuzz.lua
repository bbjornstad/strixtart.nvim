local fenv = require("env.fz")
local key_fuzz = require("keys.fz")
local key_shortcut = require("keys.shortcut")
local key_yank = require("keys.yank")
local mapx = vim.keymap.set
local fz = require("util.fz")

local fza = fz.fza
local fzf_dirs = fz.fzd
...
return {
  ...
  },
  {
    ...
    opts = {
      winopts = vim.tbl_deep_extend("force", fenv, {
        title = "querying: ",
        title_pos = "left",
        height = 0.88,
        width = 0.88,
        ...
        on_create = function()
          ...
          vim.keymap.set("t", "<C-y>", "<C-y>", { buffer = true })
          vim.keymap.set("t", "<C-g>", "<C-g>", { buffer = true })
          vim.keymap.set("t", "<C-h>", "<C-h>", { buffer = true })
          vim.keymap.set("t", "<C-x>", "<C-x>", { buffer = true })
          vim.keymap.set("t", "<C-h>", "<C-h>", { buffer = true })
          vim.keymap.set("t", "<C-o>", "<C-o>", { buffer = true })
      ...
      actions = {
        files = {
          ...
          --     selected = type(selected) ~= "table" and { tostring(selected) }
          --       or selected
    ...
    keys = {
      ...
      },
      {
        ...
        mode = { "n", "v", "i" },
        silent = true,
        desc = "fz:buf| fs |=> lines completion",
      },
      {
        "<C-x><C-S-b>",
        function()
      ...
      {
        key_shortcut.fm.files.data,
        fza("files", { cwd = vim.fs.normalize(vim.fn.stdpath("data")) }),
        mode = "n",
        desc = "fz:| fs |=> package data",
      },
    },
  },
  {
